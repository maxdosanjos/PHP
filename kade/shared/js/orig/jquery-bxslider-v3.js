...
(function(a) {
	a.fn.bxSlider = function(b) {
		...
		function W(b, c) {
			if (c == "left") {
				var d = a(".pagerslide", h).eq(b).position().left
			} else if (c == "top") {
				var d = a(".pagerslide", h).eq(b).position().top
			}
			return d
		}
		function V() {
		...
		function S(c) {
			...
			if (b.buildPager) {
				...
				for (var i = 1; i <= e; i++) {
					f += '<a href="" class="pager-link pagerslide-' + i + '">'
							+ i + "</a>"
				}
			} else if (c == "short") {
		...
		function K() {
			if (b.auto) {
				...
				b.tickerSpeed *= 10;
				a(".pagerslide", h).each(function(b) {
					A += a(this).width();
					B += a(this).height()
				});
		...
		this.destroyShow = function() {
			...
			e.children().removeAttr("style").not(".pagerslide").remove();
			g.removeClass("pagerslide")
		};
