...
//The slideshow starts when all the images are loaded
	...
	if(elem.length!=0){
		...
		if (($.browser.msie && $.browser.version < 9) || opts.loader == 'bar') {	//IE8- has some problems with canvas, I prefer to use a simple loading bar in CSS
			...
			canvas.css({'position':'absolute', 'left':0, 'z-index':80, 'height':opts.barStroke, 'width':0, 'background-color':opts.loaderColor});
		} else {
			...
			canvas.setAttribute("style", "position:absolute; z-index:81; "+opts.piePosition);
			var rad;
			var radNew;
		...
		if(pagination==true) {
			...
			for (li = 0; li < amountSlide; li++){
				$('#pix_pag_ul').append('<li id="pag_nav_'+li+'" style="position:relative; z-index:81"><span><span>'+li+'</span></span></li>');
				if(opts.thumbs==true) {
	...
	function nextSlide(nav){    //funzione per il fading delle immagini
		...
		switch(fx){
			...
			while(cycle < blocks){
				arr.push(cycle);
				delay.push(cycle);
				elem.append('<div class="diapoappended" style="display:none; overflow:hidden; position:absolute; z-index:79" />');
			...
			$.each(arr, function(index, value) {
				...
				function diapoeased() {
					...
					if($('.diapoeased').length==blocks){
						...
						selector.eq(i).show().css('z-index','78').addClass('diapocurrent');
